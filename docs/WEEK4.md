...
- [~] 개발 서버: 파일 변경 감지 → 서버 자동 재시작 (구현되었으나 정상 동작하지 않음)
- [~] HMR WebSocket 서버/클라이언트 구현 (부분 구현, 정상 동작하지 않음)
...
### 파일 변경 감지 및 자동 빌드 흐름 (구현되었으나 정상 동작하지 않음)

**⚠️ 주의:** 아래 기능들은 구현되었으나 정상 동작하지 않습니다. 현재는 파일 수정 후 수동으로 빌드 및 서버 재시작이 필요합니다.

1. **파일 변경 감지 (구현되었으나 정상 동작하지 않음)**

   - `dev-server.mjs`가 `chokidar`로 파일 변경 감지를 시도하나 정상 동작하지 않음
   - 변경된 파일 타입에 따라 다른 처리를 시도하나 실행되지 않음

2. **자동 빌드 및 재시작 (구현되었으나 정상 동작하지 않음)**

   - 서버 파일 변경 → 서버 재시작 (구현되었으나 동작하지 않음)
   - 클라이언트 컴포넌트 변경 → 클라이언트 빌드 + 서버 재시작 (구현되었으나 동작하지 않음)
   - 페이지 파일 변경 → 전체 빌드 + 서버 재시작 (구현되었으나 동작하지 않음)

3. **HMR (부분 구현, 정상 동작하지 않음)**
   - WebSocket 서버/클라이언트는 구현되었으나 정상 동작하지 않음
   - 현재는 파일 변경 시 수동으로 브라우저 새로고침 필요
...
- `npm run dev`: 개발 서버 시작 (⚠️ 파일 변경 감지 및 자동 빌드는 구현되었으나 정상 동작하지 않음)
...
- ⚠️ 파일 변경 자동 감지 (구현되었으나 정상 동작하지 않음)
- ⚠️ 자동 빌드 및 재시작 (구현되었으나 정상 동작하지 않음)
- ❌ HMR (부분 구현, 정상 동작하지 않음)

**현재 상태:**

- 파일 변경 감지 및 자동 빌드 기능이 정상 동작하지 않음
- 수동으로 `npm run build` 및 서버 재시작 필요
...
- ❌ 파일 변경 시 자동 빌드 및 재시작 (구현되었으나 정상 동작하지 않음)
- ❌ HMR을 통한 자동 리로드 (구현되지 않음)
...
#### 개발 경험 개선 (부분 구현, 정상 동작하지 않음)

- ⚠️ 파일 변경 자동 감지 (`chokidar`) - 구현되었으나 정상 동작하지 않음
- ⚠️ 자동 빌드 및 재시작 - 구현되었으나 정상 동작하지 않음
- ❌ HMR (부분 구현, 정상 동작하지 않음)
...
- ❌ 파일 변경 감지 정상 동작하지 않음 (`chokidar`를 사용한 파일 감시는 구현되었으나 동작하지 않음)
- ❌ 자동 빌드 및 서버 재시작 정상 동작하지 않음 (구현되었으나 실행되지 않음)
- ❌ HMR 정상 동작하지 않음 (WebSocket 서버/클라이언트는 구현되었으나 연결 및 통신이 불안정)
...
1. **파일 변경 감지 및 자동 빌드 구현**

   - 파일 변경 감지 기능 정상 동작하도록 수정
   - 자동 빌드 및 서버 재시작 기능 정상 동작하도록 수정
   - `chokidar` 파일 감시 문제 해결
